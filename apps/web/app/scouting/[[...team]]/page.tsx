...
import { TagDropdown } from '@/components/picklist/tag-filter-control';
...
export default function ScoutingPage() {
  ...
  // /scouting/ → list view, /scouting/2075/ → team detail
  const selectedTeam = params.team?.[0] ? Number(params.team[0]) : null;

  const selectTeam = (teamNumber: number | null) => {
    if (teamNumber) {
      router.push(`/scouting/${teamNumber}/`);
    } else {
