...
export function RankStats({ tbaRank, epaRank, rankDelta, children, className = '' }: RankStatsProps) {
    return (
        <span className={`flex items-center gap-1.5 text-[10px] ${className}`}>
            ...
            {rankDelta !== 0 && (
                <span
                    className={`px-1.5 py-0.5 rounded-full font-medium ${deltaColor(rankDelta)}`}
                >
                    Δ{rankDelta > 0 ? '+' : ''}
                    {rankDelta}
                </span>
